...
# Буферизация записи: не вызываем drain на каждый чанк (сильно ускоряет большие файлы)
WRITE_BUFFER_HIGH_WATER = 32 * 1024 * 1024  # 32 MB
WRITE_BUFFER_LOW_WATER = 8 * 1024 * 1024    # 8 MB
            ...
            with tqdm(total=size, unit='B', unit_scale=True, unit_divisor=1024,
                ...
                while sent_total < size:
                    ...
                    if transport is not None:
                        ...
                        # Делаем drain только когда буфер реально разросся
    ...
    async def connect(self):
        ...
        try:
            ...
            self._reader, self._writer = await asyncio.wait_for(
                asyncio.open_connection(self.server_ip, self.server_port, limit=CHUNK_SIZE * 2),
            ...
            # Разрешаем буферизовать запись заметно больше, чем 1 чанк
            try:
                transport = getattr(self._writer, "transport", None)
                if transport is not None:
                    transport.set_write_buffer_limits(
                        high=WRITE_BUFFER_HIGH_WATER,
                        low=WRITE_BUFFER_LOW_WATER,
                    )
            except Exception:
                pass
